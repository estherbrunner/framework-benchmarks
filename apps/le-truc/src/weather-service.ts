...
export const emptyDaily: WeatherData['daily'] = {
  time: [],
  temperature_2m_max: [],
  temperature_2m_min: [],
  weather_code: [],
...
async function getMockData(): Promise<Omit<WeatherData, 'locationName' | 'country'>> {
  try {
...
function getMockGeocodingData(cityName: string): GeocodedLocation {
...
async function geocodeLocation(cityName: string): Promise<GeocodedLocation> {
  if (useMockData) {
    return getMockGeocodingData(cityName);
  }
  try {
    const response = await fetch(
      `${geocodingUrl}/search?name=${encodeURIComponent(cityName)}&count=1&language=en&format=json`
    );
    if (!response.ok) {
...
async function getWeatherData(lat: number, lon: number): Promise<Omit<WeatherData, 'locationName' | 'country'>> {
  if (useMockData) {
    return getMockData();
  }
  try {
